...
// V9 tries to fix th intg_a. This is quite big even for events which are empty. This must be related to the poor way in which intg_a is calculated (Based on the cumulative itnegral).
...
std::tuple<double, double, double, double, std::vector<int>, int, std::string> Integral(std::vector<double> &adc, std::vector<double> &rmsv, std::vector<double> &time,   double NSigmas = 4, TH1D *single = NULL, TH1D * single_pos = NULL, int evt = 1,TTree* tree = NULL ,TFile *WF_output = NULL, double t0 = 0, double dt = 0, const int TrigTimeIdx = 0, const int a_idx = 0, const int ab_idx = 0.) {//28 arguments, evt the 7th
  ...
  for(int i = 0; i < nentries; i++ ) {      
    ...
    if (adc[i] < - rmsv[i]*NSigmas) {
      ...
      if (TrigTimeIdx < i && i < a_idx ) {
	if (evt == 7) cout << " ADC " << adc[i] << endl;
	intg_a_store += adc[i];
	min_a.push_back(i); 
      }
  ...
  //cout << intg_a_store << endl;
  intg_a = ConvertToPhotEl(intg_a_store , dt);
  ...
  if (key_min < -NSigmas * rmsv.at(key_min_idx) && intg_a >25 &&  min_a.size() > 10 &&  TMath::Abs( key_min_idx -TrigTimeIdx) > 1000){
  ...
  cout << "Inside intg fn " << intg_a<< endl;
  return std::make_tuple(intg_a, intg_b_store, intg_bef_store, intg_aft_store, min_a, key_min_idx, evt_tag);
...
int GeneralAna_v9(string path = "", const int tot_evt = 5000){
  ...
  // /srv/beegfs/scratch/users/a/amarinei/Swan_Data/Configuration_P25/Batch_1_5
  //string prefix_path_data = "../../../Swan_Data/Configuration_P25/";
  //string path_prefix_AnaResults = "../../../Swan_TTrees/Configuration_P25/";
  ...
  string prefix_path_data = "/srv/beegfs/scratch/users/a/amarinei/Swan_Data/Configuration_P25/Batch_2/NegScan/";
  string path_prefix_AnaResults = "/srv/beegfs/scratch/users/a/amarinei/Swan_TTrees/Configuration_P25/Batch_2/NegScan/";
  ...
  for (int evt = 0; evt < ampl_TPC->size(); evt++ ){
    ...
    cout << " In main: " << evt<< " intg_a " << intg_a <<   endl;
