...
// wf samples are stored in the negative
// WAVEFORM IS PLOTTED IN THE POSITIVE
...
void TTreeUnfold(bool SaveWFPlot = false, string type = "Empty"){
  ...
  path_store.push_back("/srv/beegfs/scratch/users/a/amarinei/Swan_TTrees/Configuration_P25/Batch_2/NegScan/C4600_LSF2550_ThGUp2250_ThGDnGND_PMeshNeg700_TPC1650_p2_0/");
  
  TH1D *h1 =  new TH1D("h1","", 100000, -20, 80);
  TH1D *h1_cum =  new TH1D("h1_cum","", 100000, -20, 80);
  h1_cum->SetLineColor(kRed);
  TH1D *h_rms =  new TH1D("h_rms","", 100000, -20, 80);
  TH1D *h_rms_dump =  new TH1D("h_rms_dump","", 1000, -0.1,0.1);
  TH1D *h_intg_a =  new TH1D("h_intg_a",";Number of photons per event;", 50, -0.2,1000);
  ...
  std::vector<double> *cum_vect = new std::vector<double>;
  ...
  tree->SetBranchAddress("cum_vect", &cum_vect);
  tree->GetEntry(0);
  ...
  //tree->SetBranchStatus("cum_vect", 1);
  std::vector<double> time(100000,0);
  cout << " cum_vect " << cum_vect->size()<< endl;
  ...
  int tot_evt = 5000;
  cout << " tot type" << tot_type << endl;
  int spec_evt = 4;
  
  //if (tot_evt > tot_type) tot_evt = tot_type;
  //for (int iEntry = spec_evt; iEntry < spec_evt+1; iEntry++){
  
  for (int iEntry = 0; iEntry < tot_evt; iEntry++){
    if (iEntry > tot_type) break;
    ...
    if (type == "Signal") {
      ...
      int dum_evt = Signal;
      cout << "Signal evt"<<  Signal << endl;
      tree->GetEntry(Signal);
      h1->SetTitle(Form("Evt %d | %.02f * rms | intg_a = %f , min cts = %zu",Signal, NSigmas, intg_a, min_cts_a->size()));
    }
    else if(type == "Empty"){  
	int dum_evt = Empty;
	cout << "Empty evt"<<  Empty << endl;
	tree->GetEntry(Empty); 
	h1->SetTitle(Form("Evt %d | %.02f * rms | intg_a = %f , min cts = %zu",Empty, NSigmas, intg_a, min_cts_a->size()));
      }
    

    h_intg_a->Fill(intg_a);
    hIntg_a_vs_hits->Fill(intg_a,min_cts_a->size());
	...
	//h1_cum->SetBinContent(j+1, -cum_vect->at(j));
	...
	wtf->SaveAs(Form("Pdfs/Empty/evt_%d.pdf", iEntry));
      }
