...
// this code is exploring defintions for the waveform analysis. 
...
void SignalAna(bool SaveWFPlot = false){
  ...
  path_store.push_back("../../../Swan_TTrees/Configuration_P25/C3800_LSF1750_ThGUp1450_ThGDnGND_PMesh0_TPC650_p1_0/");
  ...
  for (int iEntry = 0; iEntry < tot_evt ; iEntry++){
    ...
    intg_a_st = ConvertToPhot(dummy_intg[2],dt);
    intg_a.push_back(intg_a_st);
   
    
    
    intg_b.push_back(ConvertToPhot(dummy_intg[3],dt));
    h_intg_a->Fill(intg_a_st);
