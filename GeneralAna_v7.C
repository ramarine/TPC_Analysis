...
//compared to GeneralAna_v6 this takes into account the absolute time from each events and it is filled to the tree
...
BinaryComponents read_binary_3(TTree *tree, string path ="/Configuration_18/C3500_LSF1480_ThGUp1400_ThGDnGND_PMesh100_TPC1650_test", string path_prefix_data = "../../Swan_Data",string path_prefix_AnaResults = "../../Swan_TTrees" , const int tot_evt = 1 , TFile * WF_output = NULL){
  ...
  TBranch *B_t_evt =  tree->Branch("t_evt", &t_evt);
  ...
  for (int evt = 0; evt<tot_evt; evt++){
    ...
    tree->Fill();
    cout << setprecision(10) << " t_evt " << t_evt << endl;
  }
...
std::tuple<double, double, double, double, std::vector<int>, int, std::string> Integral(std::vector<double> &adc, std::vector<double> &rmsv, std::vector<double> &time,   double NSigmas = 4, TH1D *single = NULL, TH1D * single_pos = NULL, int evt = 1,TTree* tree = NULL ,TFile *WF_output = NULL, double t0 = 0, double dt = 0, const int TrigTimeIdx = 0, const int a_idx = 0, const int ab_idx = 0.) {//28 arguments, evt the 7th
  ...
  double intg_b_store = ConvertToPhot((Cum->GetBinContent(nentries)-Cum->GetBinContent(TrigTimeIdx))/(IntgNormAft/IntgNormA),dt);
  ...
  if (key_min < -NSigmas*rmsv.at(key_min_idx) && intg_a_store >10 && min_a.size() > 7) {
    evt_tag = "signal";
  }
  else evt_tag = "empty";
...
int GeneralAna_v7(string path = ""){
  ...
  auto bin1 = read_binary_3(tree, path, prefix_path_data, path_prefix_AnaResults, tot_evt ,WF_output);
