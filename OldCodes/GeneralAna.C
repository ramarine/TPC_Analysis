...
std::tuple<std::vector<std::vector<double>>, std::vector<std::vector<double>>, std::vector<int>, double,double,const int, const int> read_binary_3( string path ="/Configuration_18/C3500_LSF1480_ThGUp1400_ThGDnGND_PMesh100_TPC1650_test", string path_prefix_data = "../../Swan_Data",string path_prefix_AnaResults = "../../Swan_TTrees" , const int tot_evt = 1 , TFile * WF_output = NULL){
...
//  vector<vector<double>> rms_TPC;
    ...
    cout << " i crash just before the return " << endl;
 return std::make_tuple(ampl_TPC, rms_TPC, min_idx ,t0, dt, trig_idx, nentries);
}
...
//     string path = "Configuration_18/C3500_LSF1480_ThGUp1400_ThGDnGND_PMesh800_ii_TPC1650/";  const int tot_evt = 8000;
        string path = "Configuration_18/C3500_LSF1480_ThGUp1400_ThGDnGND_PMesh0_TPC1650/";  const int tot_evt = 6000;
...
//    std::tuple <vector<vector<double>>, double, double, int, int> tup;
    ...
    auto tup = read_binary_3(path,prefix_path_data,path_prefix_AnaResults, tot_evt ,WF_output );//return ampl_tpc, rms_tpc, min_idx vector, t0, dt, trig_idx, nentries
    
    std::vector<std::vector<double>> ampl_TPC= std::move(get<0>(tup));
    std::vector<std::vector<double>> rms= std::move(get<1>(tup));
    std::vector<int> min_idx =  std::move(get<2>(tup));
    const double  t0 = std::move(get<3>(tup));
    const double  dt = std::move(get<4>(tup));
    int const  nentries = std::move(get<6>(tup));
    ...
    int const TrigTimeIdx = std::move(get<5>(tup));
