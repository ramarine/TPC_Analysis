...
//compared to GeneralAna_v3 this outputs also integral information in the trees
//massive bug is fixed and now only 9gb of ram are filled instead of 16

// the integrals are converted to the number of photons normalized to the time of window/time of 
//The integral function computes the integral of any event be it empty or signal
// the integral branches are filled for both signal and empty events
//the waveform sum outputs the sums in terms of voltages. There is no conversion nor normalisation made to those plots

// this works with config P24. The main differents come from the event defintiions
...
const int NPeds = 200;
const double spark_tresh = -0.200;//1.2
const double  spark_tresh_gl = -0.200;//1.2
const double  up_spark_tresh_gl = 0.2;//0.2
...
std::tuple<double, double, double, double, std::vector<int>, int, std::string> Integral(std::vector<double> &adc, std::vector<double> &rmsv, std::vector<double> &time,   double NSigmas = 4, TH1D *single = NULL, TH1D * single_pos = NULL, int evt = 1,TTree* tree = NULL ,TFile *WF_output = NULL, double t0 = 0, double dt = 0, const int TrigTimeIdx = 0, const int a_idx = 0, const int ab_idx = 0.) {//28 arguments, evt the 7th
  ...
  if (key_min < -NSigmas*rmsv.at(key_min_idx) && intg_b_store >0 && min_a.size() > 0) {
    evt_tag = "signal";
  }
  else evt_tag = "empty";
...
int GeneralAna_v6(string path = ""){
  // path has to be of the form: C3500_..._TPC1650
  //i.e. wihtout any /
  const int tot_evt =4900;
  ...
  string prefix_path_data = "../../../Swan_Data/Configuration_P24/";
  string path_prefix_AnaResults = "../../../Swan_TTrees/Configuration_P24/";
  ...
  for (int evt = 0; evt < ampl_TPC->size(); evt++ ){
    //  if ( path == "C3800_LSF1750_ThGUp1400_ThGDnGND_PMesh427_TPC1370_p1_0" & (evt ==  85 || evt == 144 || evt == 252 || evt == 363) ) continue;
    br_evt = evt;
