...
void MergeHistos_v2(){
  ...
  //
	...
	for (int z(0);  z< tree->GetBranch("Signal")->GetEntries(); z++){
	  ...
	  intg_ab_store.at(i).push_back(intg_ab*phot_to_volt/(-con_fact));
	  intg_b_store.at(i).push_back( (intg_ab - intg_a) * phot_to_volt/(-con_fact) );
	  ct.at(i) +=1;
	}
      }
    }
    ...
    for (int i (0); i < intg_a_store.size(); i++){
      ...
      for (int j(0); j < intg_ab_store.at(i).size(); j++){
	if (intg_ab_store.at(i).at(j) != 0 && intg_ab_store.at(i).at(j) < -20 && intg_a_store.at(i).at(j) < -200)ct++;
      }
      norm_b.push_back(ct);
      ct =0;
    }
    ...
    for (int i(0); i <file_store.size(); i++){
      ...
      AbsEL_Only_ab[i] = (SumOfWFSum_ab[i]-ThGemContribution_ab);//*norm_a.at(i)/norm_b.at(i);
      AbsEL_Only_ab[i] = (SumOfWFSum_ab[i]-ThGemContribution_ab)*norm_a.at(i)/norm_b.at(i);
      cout << " norm_a " << norm_a.at(i) << " norm_b " << norm_b.at(i) << " norm_a/norm_b "<<norm_a.at(i)/norm_b.at(i) << endl;
    }
    

    
    TH1D* h_intg_a = new TH1D ("h_intg_a","h_intg_a", 100, -1000, 200);
    TH1D* h_intg_b = new TH1D("h_intg_b", "",100,-1000,200);
    TH2D* h_intg_a_ab = new TH2D ("h_intg_a_ab","h_intg_a; a; ab;", 100, -1000, 200, 100, -1000, 200);
    TH1D* h_intg_ab = new TH1D("h_intg_ab", "",100,-1000,200);

    int evt_show = 12;
    for (int i(0); i<intg_a_store.at(evt_show).size(); i++){
      h_intg_a->Fill(intg_a_store[evt_show][i]);
      h_intg_b->Fill(intg_b_store[evt_show][i]);
      h_intg_ab->Fill(intg_ab_store[evt_show][i]);
      h_intg_a_ab->Fill(intg_a_store[evt_show][i], intg_ab_store[evt_show][i]);
    }
  ...
  //Plots made to correct the b region
  ...
  TCanvas * c_intg_a = new TCanvas("c_intg_a","c_intg_a", 800, 600);
  c_intg_a->Divide(2,2);
  c_intg_a->SaveAs(Form("intg_mesh%d.root",evt_show ));
